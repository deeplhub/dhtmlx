...
function() {
    ...
    function n(t, i, a) {
        ...
        var r = "";
        ////console.info("B============="+(t.content ? "": t.text));
...
scheduler._click = {
    ...
    dhx_cal_next_button: function(e, t) {
    	//debugger;
    	//console.info(scheduler.date.add(scheduler.date[scheduler._mode + "_start"](scheduler._date), t || 1, scheduler._mode));
...
//这里还是个未知领域，有谁知道请加（QQ:365642807）探讨
scheduler.addEventNow = function(e, t, i) {
	console.info("走了这个方法scheduler.addEventNow = function(e, t, i),//这里还是个未知领域，有谁知道请加（QQ:365642807）探讨");
//	debugger;
    var s = {};
    ...
    //console.info("F1================="+s.text+"========="+this.locale.labels.new_event);
    s.text = s.text || this.locale.labels.new_event,
   // console.info("F2================="+s.text);
    
    //console.info("F3================="+s.warn_text);
    s.warn_date = s.warn_date || "",
    //console.info("F3================="+s.warn_text);
    s.scheduler_type = s.scheduler_type || "",
    //console.info("F4================="+s.scheduler_type);
...
scheduler.locale = {
    ...
    labels: {
        ...
        //xhai start
        section_warn_date: "warn information",
        section_scheduler_type: "transaction type",
        //end
        full_day: "Full day",
...
scheduler.config = {
    ...
    time_step: 5,
    start_on_monday: 1,
    first_hour: 6,//日程开始时间标记（这里只能是整数，不能带冒号或06什么来着）
    last_hour: 23,//日程结束时间标记（这里只能是整数，不能带冒号或06什么来着）
    readonly: !1,
    drag_resize: 1,
    drag_move: 1,
    drag_create: 1,
    ...
    //这里比较关键，如果想让页面添加你你想要的文本框或下拉框时，这里必须要加上一个，要不然不会显示
    lightbox: {
        sections: [{
            name: "description",
            height: 200,
        ...
        },
        {
            ...
            height: 72,
            type: "time",
            map_to: "auto"//这个目前只发现和下拉时间结合使用，并不其意义。如不用时间就不用auto
        },
        //xhai start
        {
        	 name: "scheduler_type",//标题文字信息
             height: 72,
             map_to: "scheduler_type",//接收的值
             type: "schedulerType",//scheduler.form_blocks保持一至
             focus: !0
        },
        {
       	 name: "warn_date",//标题文字信息
            height: 72,
            map_to: "warn_date",
            type: "warnDate",//scheduler.form_blocks保持一至
            focus: !0
       }
        //end
        ]
...
//初始化页面模版所用的信息，如果想调整页面上的显示信息时调整这块就行了
scheduler.init_templates = function() {
    ...
    s(scheduler.templates, {
        ...
        event_text: function(e, t, i) {
        	//console.info("G1===="+i.text+"===="+i.warn_date+"===="+t.scheduler_type);
            return i.text
        },
        //xhai start
        /*event_warn: function(e, t, i) {
        	////console.info("G2============="+i.text+"============"+i.warn);
            //return i.warn
            return i.warn_text
        },*/
        //end
...
//这里是启动时所加载的函数,如果后面想得到数据这先必须先有数据，这里的变量名和数据库返回的变量名一致
//源码：scheduler.addEvent = function(e, t, i, s, a) {
scheduler.addEvent = function(e, t, i, w, st, s, a) {
	//debugger;
    if (!arguments.length) return this.addEventNow();
    var n = e;
    //源码：1 != arguments.length && (n = a || {}, n.start_date = e, n.end_date = t, n.text = i, n.id = s),
    1 != arguments.length && (n = a || {}, n.start_date = e, n.end_date = t, n.text = i, n.warn_date = w, n.scheduler_type = st, n.id = s),
    //console.info("I2=="+n.text+"==="+n.warn_date+"==="+n.scheduler_type);
    n.id = n.id || scheduler.uid(),
    n.text = n.text || "",
    ////console.info("I3==============="+n.text);
    //xhai start
    n.warn_date = n.warn_date || "",
    n.scheduler_type = n.scheduler_type || "",
    //end
    //console.info("I4==============="+n.warn_date);
   // console.info("I5==============="+n.scheduler_type);
...
scheduler.getEvent = function(e) {
	console.info("scheduler.getEvent ============="+e);
...
//页面加载时就开始执行此方法查询，删除，更新，添加或点击时都用到此方法;此方法是在dhtmlxEventable = function(obj)方法后执行
...
//这里开始给变量赋值，这里比较关键，如果其他地方调用没有值就找这个地方看看有没有赋值
scheduler._init_event = function(e) {
	//debugger;
	//console.info("M1==============="+e.text);
    e.text = e.text || e._tagvalue || "",
    //xhai start
    e.warn_date = e.warn_date || e._tagvalue || "",
    e.scheduler_type = e.scheduler_type || e._tagvalue || "",
    //end
   // console.info("M2==============="+e.text);
...
scheduler.form_blocks = {
    ...
    schedulerType: {
    	render: function(e) {
    		//debugger;
    		var str = "<div class='dhx_cal_ltext' style='height:20'>" +
	 			"<select style='margin-left:49px; width:98px'>" +
	 				"<option id='defuntOption' value='0'>-请选择-</option>" +
        ...
        set_value: function(e, t, i, s) {
            ...
            if(t == "" || t == null){
        		document.getElementById("defuntOption").selected=true;
        	}
            console.info("A5============"+t);
        },
        get_value: function(e, t) {
        	console.info("A4============"+e.firstChild.value);
            return e.firstChild.value
        },
        focus: function(e) {
            var t = e.firstChild;
    ...
    warnDate: {
    	render: function(e) {
    		var str = "<div style='height:30px;width: 300px;padding-top:0px;padding-left: 58px;font-size:inherit;' class='dhx_cal_ltext'>" +
    		"提前<input id='day' type='text' style='width:33px;height:22px;'>天,<input id='hour' type='text' style='width:33px;height:22px;'>小时,<input id='minute' type='text' style='width:33px;height:22px;'>分钟" +
    		"</div>"
            return str;
        },
        set_value: function(e, t, i, s) {
        	...
        	//debugger;
        	if(t != "" && t != null){
        		var oldTime = (new Date(i.end_date)).getTime(); //得到毫秒数
            	
            	var day = parseInt((oldTime-t)/1000/60/60/24);//算出天数
            	var hour = parseInt((oldTime-t)/1000/60/60-(24*day));//算出小时数
            	var minute = parseInt((oldTime-t)/1000/60-(hour+24*day)*60);//算出分钟数
            	
            	e.childNodes.item(1).value = day || 0
            	e.childNodes.item(3).value = hour || 0
            	e.childNodes.item(5).value = minute || 0
        	}else{
//        		e.childNodes.item(1).value = ""
//            	e.childNodes.item(3).value = ""
//            	e.childNodes.item(5).value = ""
				e.childNodes.item(1).value = 0
				e.childNodes.item(3).value = 0
				e.childNodes.item(5).value = 0
        	}
        },
        get_value: function(e, t, i) {
        	//源码
        	//return e.firstElementChild.value
        	//e.childNodes.item(1).value:这里的查找html标签，返回的却是数组，如果标签增加或删除了都要做更改的
        	
        	var warnArr = "";
        	for(var i = 0; i<e.childNodes.length; i++){
				if(typeof(e.childNodes.item(i).value) != "undefined" && e.childNodes.item(i).value != ""){
					warnArr += e.childNodes.item(i).value + ",";
				}
        	}
        	//debugger;
        	return warnArr;
...
scheduler.save_lightbox = function() {
    ...
    var isNo = true;
    if(e.warn_date != ""){
    	var arr = e.warn_date.split(",");
    	if(arr.length-1 == 3){//3：代表3个文本框或都3个值都行
    	    var startTime = (new Date(e.start_date)).getTime(); //得到毫秒数
    	    var endTime = (new Date(e.end_date)).getTime(); //得到毫秒数
    	    var day = parseInt((endTime-startTime)/1000/60/60/24);//算出天数
    	    if(day < arr[0]){
    	    	alert("超过结束时间1");
	        	isNo = false;
	        	return;
    	    ...
    	    var hour = parseInt((endTime-startTime)/1000/60/60-(24*day));//算出小时数
    	    if(hour < arr[1] && arr[1] >= 60){
    	    	alert("超过结束时间2");
            	isNo = false;
            	return;
    	    }
    	    
    	    var minute = parseInt((endTime-startTime)/1000/60-(hour+24*day)*60);//算出分钟数
    	    if(minute < arr[2] && arr[2] >= 60){
    	    	alert("超过结束时间3");
            	isNo = false;
            	return;
    	    }
...
scheduler._update_callback = function(e, t) {
	console.info("现在执行了这个scheduler._update_callback函数，这里还是个未知领域，有谁知道请加（QQ:365642807）探讨");
    var i = scheduler._xmlNodeToJSON(e.firstChild);
    //console.info(i._tagvalue+"P1==================="+i.text);
    i.text = i.text || i._tagvalue,
    //xhai start
//    i.warn_date = i.warn_date || i._tagvalue,
//    i.scheduler_type = i.scheduler_type || i._tagvalue,
    //end
    //console.info("P2==================="+i.text+"======"+i.warn_date);
    //console.info("P3========="+i.text+"======"+i.warn_date+"======"+i.scheduler_type);
